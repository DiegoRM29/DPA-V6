...
const App = (() => {
  ...
  const FORECAST_STATE = {
    ...
    adjustPromo: true, // si hay promos cargadas, usar serie deflactada por default
...
- Español, conciso, máximo 400 palabras.
- HTML: <h4>, <strong>, <em>, <ul>, <li>.
- Estructura obligatoria (en este orden, todas las secciones):
  <h4>Lectura del pronóstico</h4> 2-3 frases con tendencia y magnitud esperada con números concretos.
  <h4>Estacionalidad</h4> Si se detectó, identifica meses pico y bajos con porcentajes; si no, indícalo.
  <h4>Riesgos del pronóstico</h4> Confianza estadística (RMSE) y supuestos a vigilar.
  <h4>Acciones sugeridas</h4> Lista de 2-3 decisiones concretas de negocio (inventario, promo, precio) con razón cuantitativa.
...
<forecast>${JSON.stringify(ctx, null, 1)}</forecast>`;
      ...
      const r = await runAITask('forecast', system,
        'Resumen ejecutivo del pronóstico.', 1100);
      renderForecastAIState('done', {
        html: r.html,
        meta: `$${r.cost.toFixed(4)} · ${r.tokensIn + r.tokensOut} tokens`
      });
    } catch (e) {
