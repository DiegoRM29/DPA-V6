...
const App = (() => {
  ...
  // Sincroniza el SKU seleccionado en el filtro de Elasticidad con el dropdown
  // de la curva y la tarjeta de información (sin re-procesar el dataset).
  function syncElasticitySkuSelection(skuId) {
    ...
    if (skuId) {
      ...
      const opt = [...sel.options].find(o => o.value === skuId);
      if (opt) {
        sel.value = skuId;
        drawCurve(skuId);
        renderElastSkuInfoCard(skuId);
      } else {
        // El SKU no tiene curva disponible — mostrar la info card de todas formas
        renderElastSkuInfoCard(skuId);
      }
    } else {
      // Sin filtro: volver al primer SKU del dropdown (top por revenue)
      if (sel.options.length && sel.options[0].value) {
        drawCurve(sel.options[0].value);
        renderElastSkuInfoCard(sel.options[0].value);
      }
    }
  }
  ...
  function renderElasticity() {
    ...
    // ====== NUEVA GRÁFICA: elasticidad promedio por categoría ======
    // Reemplaza al histograma genérico. Es más accionable porque muestra el patrón
    // a nivel categoría (qué categorías son inelásticas vs cuáles son elásticas).
    // Ordenado ascendente por |elast|: izquierda = más poder de pricing.
    destroyChart('elastByCategory');
    const categoryStats = new Map(); // cat → {elasts: [], skus: 0}
    const elastSource = showAll ? skus : propias;
      ...
      charts.elastByCategory = new Chart(catCanvas, {
        ...
        data: {
          ...
          datasets: [{
            ...
            backgroundColor: catArr.map(c => colorFor(c.avg)),
            borderColor: catArr.map(c => borderFor(c.avg)),
            borderWidth: 1.5,
            borderRadius: 6,
            _raw: catArr // guardamos para tooltip
          }]
        },
        options: {
          indexAxis: 'y',
    ...
    if (curveSkus.length) {
      ...
      sel.onchange = e => {
        // Sincronizar también el filtro SKU de la sección (para mantener consistencia)
        if (SECTION_FILTERS.elasticity) {
          SECTION_FILTERS.elasticity.sku = e.target.value;
      ...
      // Si hay un SKU filtrado en la sección, intentar usarlo; si no, top por revenue
      const filteredSku = SECTION_FILTERS.elasticity?.sku;
      const initialSku = (filteredSku && filteredSku !== 'all' &&
                          curveSkus.find(s => String(s.sku) === filteredSku))
        ? filteredSku
        : curveSkus[0].sku;
      sel.value = initialSku;
      drawCurve(initialSku);
      renderElastSkuInfoCard(initialSku);
    } else {
