...
const App = (() => {
  ...
  function processData(filters) {
    ...
    // Si hay PROMO_DATA cargado, usar las fechas reales de promo para identificar
    // los periodos. Si no, usar heurística de variación de precio.
    let postPromo = null;

    if (PROMO_DATA && PROMO_DATA.bySkuIndex.size > 0) {
      // Modo preciso: usar los registros de promo para identificar antes/durante/después
      ...
      for (const sku of skusWithPromo.slice(0, 300)) {
        ...
        // Usar la primera promo del SKU como referencia (no la última,
        // así dejamos margen para periodo después en el histórico)
        const firstPromo = promoList[0];
        const pStart = firstPromo.start.getTime();
        const pEnd = firstPromo.end.getTime();
        // Ventana ampliada: 2x la duración de la promo, para capturar más después
      ...
      if (samples.antes.length >= 3 && samples.durante.length >= 3) {
        ...
        // Umbral bajado de 3 a 2 para "después" — muchas veces la promo está cerca
        // del final del histórico y hay pocos puntos post.
        const despues = samples.despues.length >= 2 ? avg(samples.despues) : null;
        postPromo = {
          antes, durante,
          despues: despues !== null ? despues : antes,
          liftDurante: antes > 0 ? (durante / antes - 1) * 100 : 0,
          liftDespues: (despues !== null && antes > 0) ? (despues / antes - 1) * 100 : 0,
          samples: samples.antes.length + samples.durante.length + samples.despues.length,
          source: 'promo_data',
          hasPost: despues !== null,
          samplesAfter: samples.despues.length
        };
      }
    }

    // Fallback heurístico: detectar SKUs con variación de precio y estimar periodos
    if (!postPromo) {
      ...
      if (promoSkus.length && monthly.length >= 2) {
        ...
        let nAntes = 0, nDurante = 0, nDespues = 0;
        for (const sku of promoSkus.slice(0, 200)) {
          ...
          // Permitir que promoIdx sea hasta length-2 (no length-1) para que haya al menos 1 punto después
          ...
          // "Después": cualquier punto a partir del fin del periodo durante
          // (no requerir ventana específica; tomar lo que haya disponible)
          const pSlice = items.slice(Math.min(items.length, promoIdx + window));
          if (aSlice.length) { antes += aSlice.reduce((x,y) => x + y.qty, 0) / aSlice.length; nAntes++; }
          if (dSlice.length) { durante += dSlice.reduce((x,y) => x + y.qty, 0) / dSlice.length; nDurante++; }
          if (pSlice.length) { despues += pSlice.reduce((x,y) => x + y.qty, 0) / pSlice.length; nDespues++; }
        }
        if (nAntes && nDurante) {
          antes /= nAntes;
          durante /= nDurante;
          // Usar "después" si hay al menos algo (no requiere 3 SKUs)
          const hasDespues = nDespues > 0;
          despues = hasDespues ? despues / nDespues : antes;
          postPromo = {
            antes, durante, despues,
            liftDurante: antes > 0 ? (durante / antes - 1) * 100 : 0,
            liftDespues: hasDespues && antes > 0 ? (despues / antes - 1) * 100 : 0,
            samples: nAntes + nDurante + nDespues,
            source: 'heuristic',
            hasPost: hasDespues,
            samplesAfter: nDespues
          };
        }
      }
    }

    // Último fallback: tendencia general mensual
    if (!postPromo && monthly.length >= 3) {
      const sortedM = [...monthly].sort((a,b) => a.unidades - b.unidades);
      const promoMonths = sortedM.slice(-Math.max(1, Math.floor(sortedM.length * 0.2)));
      ...
      if (normalMonths.length > 0 && promoMonths.length > 0) {
        ...
        postPromo = { antes, durante, despues, liftDurante, liftDespues: 0, samples: promoMonths.length, source: 'inferred', hasPost: false };
      }
    }
  ...
  function renderDescriptive() {
    ...
    if (pp) {
      ...
      // Si no hay datos de "después", mostrar solo 2 períodos
      const labels = pp.hasPost ? ['Antes', 'Durante promo', 'Después'] : ['Antes', 'Durante promo'];
      const data = pp.hasPost ? [pp.antes, pp.durante, pp.despues] : [pp.antes, pp.durante];
      const colors = pp.hasPost
        ? ['rgba(168,168,179,0.85)', 'rgba(255,209,0,0.85)', 'rgba(255,123,0,0.85)']
        : ['rgba(168,168,179,0.85)', 'rgba(255,209,0,0.85)'];
      const borderColors = pp.hasPost
        ? ['#a8a8b3', '#FFD100', '#FF7B00']
        : ['#a8a8b3', '#FFD100'];
          ...
          plugins: {
            ...
            tooltip: { ...tooltipStyle, callbacks: {
              label: c => {
                ...
                return [base, `vs antes: ${liftFromAntes >= 0 ? '+' : ''}${liftFromAntes.toFixed(1)}%`];
              }
            } }
          },
          scales: {
      ...
      // Badge con lift de durante (más relevante que post)
...
<data>${JSON.stringify(ctx, null, 1)}</data>`;
  ...
  const FORECAST_STATE = {
    ...
    adjustPromo: true, // SIEMPRE activo: cuando hay promos cargadas, usa serie deflactada
    result: null,
    series: null
  };
  ...
  function renderForecast() {
    ...
    // Si hay promos cargadas y el toggle está activo, usar serie deflactada para entrenar
    const useDeflated = PROMO_DATA && FORECAST_STATE.adjustPromo && series.valuesDeflated;
    const trainingValues = useDeflated ? series.valuesDeflated : series.values;
    // Pasar flags de promoción al modelo ML como feature explícito
    ...
    result._usedDeflated = useDeflated;
    ...
    // Badge del modelo
    const modelMap = {
      'gradient_boosting_trees': '🌳 Gradient Boosting (ML supervisado)',
      'media_simple': '📊 Media simple (data insuficiente)',
      'naive': '· Naive'
    };
    ...
    // Resetear estado del resumen IA al cambiar de SKU / parámetros
    renderForecastAIState('empty');
  }
  ...
  function renderForecastKpis(series, result, h) {
    ...
    document.getElementById('forecastKpis').innerHTML = `
      ...
      ${PROMO_DATA ? (() => {
        const upInfo = getUpliftForSku(FORECAST_STATE.sku);
        const promoMonths = (series.monthHasPromoArr || []).filter(Boolean).length;
        return `
        <div class="kpi" style="border-color: ${upInfo.source === 'propio' ? 'rgba(80,200,120,0.3)' : 'var(--border)'};">
          <div class="kpi-label">Uplift promocional</div>
          <div class="kpi-value" style="color: ${upInfo.uplift > 1.05 ? 'var(--green)' : 'var(--text)'};">${upInfo.uplift > 1.01 ? '+' + ((upInfo.uplift - 1) * 100).toFixed(0) + '%' : 'Sin efecto'}</div>
          <div class="kpi-meta"><span>Fuente: ${upInfo.source === 'propio' ? `propio (${upInfo.detail.monthsPromo}m c/promo, ${upInfo.detail.monthsNoPromo}m sin)` : upInfo.source === 'categoria' ? 'categoría (fallback)' : upInfo.source === 'global' ? 'global (fallback)' : 'sin datos'}</span></div>
        </div>
        <div class="kpi">
          <div class="kpi-label">Meses con promo</div>
          <div class="kpi-value">${promoMonths}<span style="color:var(--text-3);font-size:14px">/${series.values.length}</span></div>
          <div class="kpi-meta"><span>Excluidos del entrenamiento</span></div>
        </div>
      `;
      })() : ''}
    `;
  ...
  function renderForecastChart(series, result, h) {
    ...
    const lastHist = series.values[histN - 1];
    // Si usamos serie deflactada, el ancla de continuidad es el último valor deflactado
    const forecastAnchor = result._usedDeflated && series.valuesDeflated
      ? series.valuesDeflated[histN - 1]
      : lastHist;
    const forecastData = [...Array(histN - 1).fill(null), forecastAnchor, ...result.forecast];

    // Estilo de puntos: rojos en meses con promo (si hay data de promos)
    const promoArr = series.monthHasPromoArr || [];
    const histPointColors = series.values.map((_, i) =>
      promoArr[i] ? '#ff4d6d' : '#FFD100');
    const histPointRadii = series.values.map((_, i) =>
      promoArr[i] ? 4 : 2);
    ...
    const datasets = [
      { label: 'Histórico', data: histData,
        ...
        pointRadius: [...histPointRadii, ...Array(h).fill(0)],
        pointHoverRadius: 5,
        pointBackgroundColor: [...histPointColors, ...Array(h).fill('#FFD100')],
        pointBorderColor: [...histPointColors, ...Array(h).fill('#FFD100')] },
      { label: result._usedDeflated ? 'Pronóstico (demanda base)' : 'Pronóstico', data: forecastData,
    ...
    // Si se usó serie deflactada, mostrar también la línea base como referencia ligera
    if (result._usedDeflated && series.valuesDeflated) {
      const baseData = [...series.valuesDeflated, ...Array(h).fill(null)];
      datasets.unshift({
        label: 'Demanda base (sin promo)',
        data: baseData,
  ...
  // Reemplaza la antigua "estacionalidad" por importancia de features del modelo ML
  function renderSeasonalChart(result) {
    destroyChart('seasonal');
    const wrap = document.getElementById('chartSeasonal').parentElement;
    // Reconstruir canvas si fue reemplazado por un div anteriormente
    if (wrap.querySelector('canvas') === null) {
      wrap.innerHTML = '<canvas id="chartSeasonal"></canvas>';
    }
    if (!result.featureImportance || !Object.keys(result.featureImportance).length) {
      wrap.innerHTML = `<div style="display:grid;place-items:center;height:100%;color:var(--text-3);font-size:12px;text-align:center;padding:20px;">Importancia de features no disponible<br><span style="color:var(--text-3);font-size:11px;">(modelo: ${result.method})</span></div>`;
      return;
    }
    // Importance: ordenar de mayor a menor
    const entries = Object.entries(result.featureImportance).sort((a,b) => b[1] - a[1]);
    const featureLabels = {
      lag1: 'Mes anterior',
      lag2: 'Hace 2 meses',
      lag3: 'Hace 3 meses',
      ...
      mean3: 'Promedio 3m',
      mean6: 'Promedio 6m',
      month_sin: 'Estacionalidad (sin)',
      month_cos: 'Estacionalidad (cos)',
      trend: 'Tendencia temporal',
      promo_now: 'Promo en mes objetivo',
      promo_prev: 'Promo mes anterior'
    };
    const labels = entries.map(([k,_]) => featureLabels[k] || k);
    const data = entries.map(([_,v]) => v);
    // Color: features de promo destacados en amarillo, lags en azul, estacionalidad en verde
    const colorFor = key => {
      if (key.includes('promo')) return 'rgba(255,209,0,0.85)';
      if (key.includes('month')) return 'rgba(80,200,120,0.85)';
      if (key.includes('lag')) return 'rgba(77,159,255,0.85)';
      if (key.includes('trend')) return 'rgba(168,168,179,0.85)';
      if (key.includes('mean')) return 'rgba(255,123,0,0.85)';
      return 'rgba(168,168,179,0.85)';
    };
    charts.seasonal = new Chart(document.getElementById('chartSeasonal'), {
      ...
      data: {
        ...
        datasets: [{
          label: 'Importancia (%)',
          data,
          backgroundColor: entries.map(([k,_]) => colorFor(k)),
          borderRadius: 4
        }]
      },
      options: {
        ...
        plugins: {
          legend: { display: false },
          tooltip: { ...tooltipStyle, callbacks: {
            label: c => `Importancia: ${c.parsed.x.toFixed(1)}% del total de splits`
          } }
        },
        scales: {
          x: { ...baseScale, ticks: { ...baseScale.ticks, callback: v => v + '%' }, beginAtZero: true },
          y: { ...baseScale, grid: { display: false }, ticks: { ...baseScale.ticks, font: { size: 10.5 } } }
        }
      }
    });
  ...
  function syncForecastFilters() {
    ...
    // Ajuste promocional: oculto si no hay promos cargadas
    const adjustGroup = document.getElementById('forecastAdjustPromoGroup');
    if (adjustGroup) {
      const parent = adjustGroup.closest('.filter-group');
      if (parent) parent.style.display = PROMO_DATA ? '' : 'none';
      adjustGroup.querySelectorAll('[data-adjust]').forEach(b => {
        const on = b.dataset.adjust === 'on';
        b.classList.toggle('active', on === FORECAST_STATE.adjustPromo);
      });
    }
  }
  ...
  function setupForecastFilters() {
    ...
    // Ajuste promocional
    bar.querySelectorAll('[data-adjust]').forEach(btn => {
      btn.addEventListener('click', () => {
        FORECAST_STATE.adjustPromo = btn.dataset.adjust === 'on';
        renderForecast();
      });
    });
    // Reset
...
<scenario>${JSON.stringify(scenario, null, 1)}</scenario>`;
  ...
  async function aiTaskExecutiveNarrative() {
    try {
      ...
      const anonymizer = createAnonymizer();
      const ctx = buildAnonymizedDatasetContext(anonymizer);
      const system = `Eres consultor senior de pricing. Escribe una narrativa ejecutiva para C-level.

REGLAS DE FORMATO CRÍTICAS:
- NO empieces con saludos. NO termines con cierres.
- Tono ejecutivo, claro. Español, máx 450 palabras.
- HTML: <h4>, <strong>, <em>, <ul>, <li>.
- Estructura:
  <h4>Estado del negocio</h4> 2-3 frases con datos.
  <h4>Oportunidades clave</h4> Lista cuantificada. Refiere SKUs por su ID (P001, P003...).
  <h4>Recomendaciones de la semana</h4> Lista accionable.

IMPORTANTE: Los SKUs son IDs anónimos (P001...), marcas (M001...), categorías (C001...). La app traducirá a nombres reales.
...
<dataset>${JSON.stringify(ctx, null, 1)}</dataset>`;
      const r = await runAITask('narrative', system,
        'Narrativa ejecutiva para dirección.', 1100, anonymizer);
...
<anomaly>${JSON.stringify(ctxItem, null, 1)}</anomaly>`;
  ...
  // Render del resumen IA del pronóstico — INLINE en la card, no en modal.
  // Estado: { empty | loading | done | error } controla qué bloque se muestra.
  function renderForecastAIState(state, opts = {}) {
    const emptyEl = document.getElementById('forecastAIEmpty');
    const contentEl = document.getElementById('forecastAIContent');
    const loadingEl = document.getElementById('forecastAILoading');
    const genBtn = document.getElementById('forecastAIGenerateBtn');
    const regenBtn = document.getElementById('forecastAIRegenerateBtn');
    const metaBadge = document.getElementById('forecastAIMeta');
    const emptyText = document.getElementById('forecastAIEmptyText');
    if (!emptyEl || !contentEl || !loadingEl) return;
    ...
    if (state === 'empty') {
      ...
      if (emptyText) {
        emptyText.innerHTML = isLLMConnected()
          ? 'Click en <strong>Generar resumen</strong> para que la IA analice este pronóstico y entregue lectura ejecutiva, identificación de estacionalidad, riesgos y 2-3 acciones de negocio recomendadas.'
          : 'Conecta una IA en <strong>Upload & Mapping → Análisis IA</strong> para habilitar el resumen ejecutivo automático del pronóstico.';
      }
      if (isLLMConnected() && genBtn) genBtn.style.display = '';
    } else if (state === 'loading') {
      loadingEl.style.display = '';
      document.getElementById('forecastAILoadingText').textContent =
        `Consultando ${PROVIDERS[LLM_STATE.provider]?.name || 'IA'}...`;
    } else if (state === 'done') {
      contentEl.style.display = '';
      contentEl.innerHTML = opts.html || '';
      if (regenBtn) regenBtn.style.display = '';
      if (metaBadge && opts.meta) {
        metaBadge.textContent = opts.meta;
  ...
  async function aiTaskForecastInterpret() {
    if (!FORECAST_STATE.result || !FORECAST_STATE.series) {
      renderForecastAIState('error', { error: 'Selecciona primero un SKU.' });
      return;
    }
    if (!isLLMConnected()) {
      renderForecastAIState('empty');
    ...
    try {
      ...
      const ctx = {
        ...
        forecast: {
          ...
          model: result.method,
          promo_adjustment_applied: !!result._usedDeflated
        },
        promo_uplift: upInfo ? {
          uplift_pct: upInfo.uplift > 1 ? +((upInfo.uplift - 1) * 100).toFixed(0) : 0,
          source: upInfo.source
        } : null,
      ...
      const system = `Eres analista de pricing especializado en pronósticos retail. Interpreta el pronóstico y entrega insights accionables.
...
- HTML: <h4>, <strong>, <em>, <ul>, <li>.
- Estructura obligatoria:
  <h4>Lectura del pronóstico</h4> 2-3 frases con tendencia y magnitud.
  <h4>Estacionalidad</h4> Si se detectó, identifica meses pico/bajos con %.
  <h4>Riesgos del pronóstico</h4> Confianza estadística y supuestos.
  <h4>Acciones sugeridas</h4> 2-3 decisiones de negocio (inventario, promo, precio) con razón cuantitativa.

IMPORTANTE: El SKU es ${skuAnon} (anónimo). La app traducirá al nombre real del producto.
...
<forecast>${JSON.stringify(ctx, null, 1)}</forecast>`;
      const r = await runAITask('forecast', system,
        'Resumen ejecutivo del pronóstico.', 1100, anonymizer);
      renderForecastAIState('done', {
        html: r.html,
        meta: `$${r.cost.toFixed(4)} · datos anonimizados`
      });
    } catch (e) {
      renderForecastAIState('error', { error: e.message || 'No se pudo completar' });
    }
  }

  function setupForecastAICard() {
    const genBtn = document.getElementById('forecastAIGenerateBtn');
    const regenBtn = document.getElementById('forecastAIRegenerateBtn');
    if (genBtn) genBtn.addEventListener('click', () => aiTaskForecastInterpret());
    if (regenBtn) regenBtn.addEventListener('click', () => aiTaskForecastInterpret());
  }
