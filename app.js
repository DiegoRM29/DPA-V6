...
const App = (() => {
  ...
  document.addEventListener('click', e => {
    ...
    if (!trigger) return;
    if (e.target.closest('[data-ai-button]') || e.target.closest('button')) return; // no interceptar botones internos
  ...
  // ============ SIDEBAR TOGGLE ============
  // Re-dimensiona charts en múltiples pasadas para evitar el bug de "gráfica cortada":
  // Chart.js no puede medir bien un canvas que estaba en display:none cuando se intentó
  // redimensionar. Disparamos resize() en RAF + setTimeout en varios momentos para garantizar
  // que cuando el contenedor sí tenga dimensiones reales, Chart.js las capture.
  function forceChartsResize() {
    const doResize = () => {
      Object.values(charts).forEach(c => {
        if (!c || !c.canvas) return;
        try {
          // Solo redimensionar charts cuyo canvas tiene dimensiones reales en el DOM
          const rect = c.canvas.getBoundingClientRect();
          if (rect.width > 0 && rect.height > 0) c.resize();
        } catch(e) {}
      });
    };
    // Disparar Chart.js' resize integrado (escucha window.resize)
    window.dispatchEvent(new Event('resize'));
    // Y varias pasadas explícitas en distintos momentos
