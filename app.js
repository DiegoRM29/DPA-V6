...
const App = (() => {
  ...
  function processData(filters) {
    ...
    if (PROMO_DATA && PROMO_DATA.bySkuIndex.size > 0) {
      ...
      for (const sku of skusWithPromo.slice(0, 300)) {
        ...
        // Usar la primera promo del SKU como referencia
        const firstPromo = promoList[0];
        const pStart = firstPromo.start.getTime();
        const pEnd = firstPromo.end.getTime();
        const windowMs = (pEnd - pStart) * 1.5; // ventana antes/después proporcional a la promo
      ...
      if (samples.antes.length >= 3 && samples.durante.length >= 3) {
        ...
        const despues = samples.despues.length >= 3 ? avg(samples.despues) : antes;
        postPromo = {
          ...
          source: 'promo_data',
          hasPost: samples.despues.length >= 3
        };
      }
    }
    ...
    if (!postPromo) {
      ...
      if (promoSkus.length && monthly.length >= 2) {
        ...
        if (nAntes && nDurante) {
          antes /= nAntes;
          durante /= nDurante;
          despues = nDespues ? despues / nDespues : antes;
          postPromo = {
            antes, durante, despues,
            liftDurante: antes > 0 ? (durante / antes - 1) * 100 : 0,
            liftDespues: antes > 0 ? (despues / antes - 1) * 100 : 0,
  ...
  function renderElasticity() {
    ...
    if (curveSkus.length) {
      ...
      sel.onchange = e => {
        ...
        renderElastSkuInfoCard(e.target.value);
      };
      drawCurve(curveSkus[0].sku);
      renderElastSkuInfoCard(curveSkus[0].sku);
    } else {
  ...
  function renderElastSkuInfoCard(skuId) {
    ...
    bodyEl.innerHTML = `
      <div>
        <div style="font-size: 10px; color: var(--text-3); text-transform: uppercase; font-weight: 600;">Elasticidad</div>
        <div class="mono" style="font-size: 15px; font-weight: 700; color: ${elastColor};">${sku.elasticidad.toFixed(2)}</div>
        <div style="font-size: 10.5px; color: var(--text-3);">${elastDir} · ${sku.elastSource === 'categoria' ? 'heredada' : 'propia'}</div>
      </div>
      <div>
        <div style="font-size: 10px; color: var(--text-3); text-transform: uppercase; font-weight: 600;">Confianza</div>
        <div class="mono" style="font-size: 15px; font-weight: 700;">${sku.confianza || '—'}</div>
        <div style="font-size: 10.5px; color: var(--text-3);">R² ${(sku.r2 || 0).toFixed(2)}</div>
      </div>
      <div>
        <div style="font-size: 10px; color: var(--text-3); text-transform: uppercase; font-weight: 600;">Precio · Margen</div>
        <div class="mono" style="font-size: 15px; font-weight: 700;">${fmt.money2(sku.precio)}</div>
        <div style="font-size: 10.5px; color: var(--text-3);">Mg ${(sku.margen*100).toFixed(1)}%</div>
      </div>
      <div>
        <div style="font-size: 10px; color: var(--text-3); text-transform: uppercase; font-weight: 600;">Demanda · Segmento</div>
        <div class="mono" style="font-size: 15px; font-weight: 700;">${({muy_alta:'Muy alta',alta:'Alta',media:'Media',baja:'Baja'}[sku.demanda] || 'Media')}</div>
        <div style="font-size: 10.5px; color: var(--text-3);">${sku.segmento || '—'}</div>
      </div>
      <div>
        <div style="font-size: 10px; color: var(--text-3); text-transform: uppercase; font-weight: 600;">Acción recomendada</div>
        <div style="font-size: 13px; font-weight: 700; color: var(--yellow);">${sku.accion || '—'}</div>
        <div style="font-size: 10.5px; color: var(--text-3);">${sku.accion_pct ? (sku.accion_pct > 0 ? '+' : '') + sku.accion_pct + '%' : ''}</div>
      </div>
      ${PROMO_DATA && upInfo ? `
        <div>
          <div style="font-size: 10px; color: var(--text-3); text-transform: uppercase; font-weight: 600;">Uplift promo histórico</div>
          <div class="mono" style="font-size: 15px; font-weight: 700; color: var(--green);">${upInfo.uplift > 1.01 ? '+' + ((upInfo.uplift - 1) * 100).toFixed(0) + '%' : 'Sin efecto'}</div>
          <div style="font-size: 10.5px; color: var(--text-3);">Elast ${sku._promoAdjusted ? 'sin sesgo promo' : 'sin promos detectadas'}</div>
        </div>
      ` : ''}
    `;
  }
...
<data>${JSON.stringify(ctx, null, 1)}</data>`;
  ...
  function renderForecast() {
    ...
    const result = holtWintersForecast(trainingValues, h, { season: 12 });
