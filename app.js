...
const App = (() => {
  ...
  const DEFAULT_FILTERS_MULTI = () => ({ window: 'all', category: [], brand: [], store: [], decay: false });
  ...
  function updateSim() {
    ...
    // Texto del cambio de demanda
    const volChangePct = ((volRatio - 1) * 100);
    const volDirectionText = volRatio > 1.02
      ? `<strong style="color:var(--green)">la demanda sube ${volChangePct.toFixed(1)}%</strong>`
      : volRatio < 0.98
      ? `<strong style="color:var(--red)">la demanda baja ${Math.abs(volChangePct).toFixed(1)}%</strong>`
      : `<strong style="color:var(--text-2)">la demanda se mantiene</strong>`;
    ...
    else if (totalProfitImpact > baseProfit * 0.05) reco = `<span style="color:var(--green);font-weight:600;">✓ Escenario favorable.</span> Con este cambio ${volDirectionText} y la utilidad unitaria ${fmt.signed((totalProfitImpact/baseProfit)*100)}. Producto de demanda <strong style="color:${demColor}">${demLabel}</strong> → impacto absoluto en P&L: <strong>${demImpact}</strong>. La elasticidad (${sku.elasticidad.toFixed(2)}) absorbe el cambio.`;
    else if (totalProfitImpact < -baseProfit * 0.05) reco = `<span style="color:var(--red);font-weight:600;">✗ Escenario desfavorable.</span> Con este cambio ${volDirectionText} y la utilidad cae ${fmt.signed((totalProfitImpact/baseProfit)*100)}. ${sku.demanda === 'muy_alta' || sku.demanda === 'alta' ? '<strong>Demanda alta amplifica la pérdida.</strong>' : 'En demanda ' + demLabel + ', el daño es limitado pero real.'}`;
    else reco = `<span style="color:var(--text-2);font-weight:600;">○ Escenario neutro.</span> ${volDirectionText} y el impacto en utilidad es marginal. Demanda ${demLabel}: considera otras palancas (cost-down, mix, otro tipo de promo).`;
  ...
  // ============ AI CHAT ASSISTANT ============
  // Asistente rule-based que responde preguntas sobre el dataset cargado.
  // Procesa lenguaje natural con keywords y devuelve respuestas estructuradas.
  const CHAT_HISTORY = [];
      ...
      if (open) {
        if (!CHAT_HISTORY.length) {
          chatBot(DATASET_BASE
            ? `¡Hola! Soy tu asistente de pricing. Puedo responder preguntas sobre tus ${fmt.num(DATASET_BASE.meta.skusTotales)} SKUs. ¿Qué quieres saber?`
            : 'Carga tu archivo CSV/Excel primero. Una vez procesado, podré responder preguntas sobre tus datos.');
        }
  ...
  function sendChatMessage() {
    ...
    setTimeout(() => {
      ...
      const response = answerQuery(text);
      chatBot(response);
      renderChatSuggestions();
    }, 400 + Math.random() * 400);
  }
  ...
  function chatBot(html) {
    CHAT_HISTORY.push({ role: 'ai', text: html });
  ...
  function renderChatSuggestions() {
    ...
    if (!DATASET_BASE) {
      ...
      suggestions = [
        '¿Cuáles son mis top 5 SKUs?',
        '¿Dónde tengo más oportunidad de subir precio?',
        '¿Qué productos están en riesgo?',
        'Resumen ejecutivo',
        '¿Qué SKUs recomendarías para promoción 2x1?',
        '¿Cuál es mi marca más rentable?',
        '¿Hay anomalías importantes?',
  ...
  function answerQuery(q) {
    const text = q.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,''); // sin acentos
    if (!DATASET_BASE) {
      return 'Carga tu archivo CSV/Excel primero (botón arriba a la izquierda en <strong>Upload &amp; Mapping</strong>). Una vez procesado, podré responder preguntas como "<em>top 5 SKUs</em>", "<em>oportunidades de subir precio</em>", "<em>resumen ejecutivo</em>", etc.';
    }
    const ds = DATASET_BASE;

    // === SALUDOS ===
    if (/^(hola|hi|hey|buenos|buenas|que tal|qué tal)/.test(text)) {
      return `¡Hola! Estoy listo para ayudarte. Tu dataset tiene <strong>${fmt.num(ds.meta.skusTotales)} SKUs</strong> sobre <strong>${fmt.num(ds.meta.filasTotales)} transacciones</strong>. Algunas cosas que puedo hacer: usa las sugerencias abajo o pregúntame en lenguaje natural.`;
    }

    // === RESUMEN EJECUTIVO ===
    if (/resumen|summary|ejecutivo|overview/.test(text)) {
      const k = ds.kpis;
      ...
      return `<strong>Resumen ejecutivo</strong><br><br>
        📊 <strong>${fmt.money(k.revenue_total)}</strong> en revenue · <strong>${fmt.money(k.utilidad_total)}</strong> en utilidad<br>
        📦 <strong>${fmt.num(k.skus)}</strong> SKUs activos · <strong>${k.marcas}</strong> marcas · <strong>${k.tiendas || '—'}</strong> tiendas<br>
        ...
        Oportunidad estimada de revenue: <strong style="color:var(--green)">+${fmt.money(ds.meta.revOportunidad)}</strong>`;
    }
    ...
    // === OPORTUNIDADES SUBIR PRECIO ===
    if (/subir precio|aumentar precio|oportunidad.*precio|incremento|captura/.test(text)) {
      const opps = ds.skus.filter(s => s.accion === 'SUBIR PRECIO').sort((a,b) => b.revenue - a.revenue).slice(0, 8);
      if (!opps.length) return 'No identifiqué SKUs con oportunidad clara de subir precio en tu dataset. Generalmente esto pasa cuando los márgenes ya son bajos o la elasticidad es alta. Tip: prueba el simulador con un SKU específico.';
      return `<strong>${opps.length} SKUs con oportunidad de subir precio</strong> (top por revenue):
        <table><thead><tr><th>Producto</th><th>Precio</th><th>Δ</th><th>Mg</th><th>E</th></tr></thead><tbody>
        ${opps.map(s => `<tr><td>${escapeHtml(s.nombre).substring(0,30)}</td><td>${fmt.money2(s.precio)}</td><td style="color:var(--green)">+${s.accion_pct}%</td><td>${(s.margen*100).toFixed(0)}%</td><td>${s.elasticidad.toFixed(2)}</td></tr>`).join('')}
        </tbody></table>
        En total son <em>${ds.skus.filter(s=>s.accion==='SUBIR PRECIO').length}</em> SKUs con esta recomendación. Ve a <strong>Recomendaciones IA</strong> para verlos todos.`;
    }

    // === PROMOS ===
    if (/promo|2x1|3x2|bundle|descuento/.test(text)) {
      const promos = ds.skus.filter(s => s.accion.startsWith('PROMO') || s.accion === 'BUNDLE').sort((a,b) => b.revenue - a.revenue).slice(0, 8);
      if (!promos.length) return 'No detecté SKUs candidatos claros para promociones agresivas. Normalmente las promos NxM funcionan mejor en productos con elasticidad alta (|E|&gt;1.5) y margen amplio (&gt;30%).';
      return `<strong>${promos.length} SKUs candidatos a promo/bundle:</strong>
        <table><thead><tr><th>Producto</th><th>Acción</th><th>Mg</th><th>E</th></tr></thead><tbody>
        ${promos.map(s => `<tr><td>${escapeHtml(s.nombre).substring(0,30)}</td><td><em>${s.accion}</em></td><td>${(s.margen*100).toFixed(0)}%</td><td>${s.elasticidad.toFixed(2)}</td></tr>`).join('')}
        </tbody></table>`;
    }

    // === RIESGO / CRITICOS ===
    if (/riesgo|critico|crítico|margen bajo|peligro|problema/.test(text)) {
      const risk = ds.skus.filter(s => s.margen < 0.10 || s.accion === 'REVISAR COSTO' || s.accion === 'DISCONTINUAR').sort((a,b) => b.revenue - a.revenue).slice(0, 10);
      if (!risk.length) return 'Buenas noticias: no detecté SKUs en riesgo crítico (margen &lt; 10% o discontinuación recomendada).';
      return `<strong>⚠ ${risk.length} SKUs en riesgo:</strong>
        <table><thead><tr><th>Producto</th><th>Mg</th><th>Acción</th></tr></thead><tbody>
        ${risk.map(s => `<tr><td>${escapeHtml(s.nombre).substring(0,30)}</td><td style="color:var(--red)">${(s.margen*100).toFixed(1)}%</td><td><em>${s.accion}</em></td></tr>`).join('')}
        </tbody></table>`;
    }

    // === ANOMALÍAS ===
    if (/anomal|alerta|raro|extrano|extraño/.test(text)) {
      if (!ds.anomalias.length || ds.anomalias[0].tipo === 'info') return 'Sin anomalías críticas detectadas. Tu data está limpia.';
      return `<strong>Anomalías detectadas:</strong><ul>
        ${ds.anomalias.slice(0, 8).map(a => `<li>${a.tipo === 'critico' ? '🔴' : a.tipo === 'warning' ? '⚠' : 'ℹ'} <code>${a.sku}</code> (${escapeHtml(a.marca)}): ${escapeHtml(a.mensaje)}</li>`).join('')}
        </ul>`;
    }

    // === MARCAS ===
    if (/marca.*rentable|mejor marca|top marca|marca.*mayor/.test(text)) {
      const top = ds.marcas.slice(0, 5);
      return `<strong>Top 5 marcas por revenue:</strong>
        <table><thead><tr><th>Marca</th><th>Revenue</th><th>% share</th></tr></thead><tbody>
        ${top.map(m => `<tr><td>${escapeHtml(m.nombre)}</td><td>${fmt.money(m.revenue)}</td><td>${(m.revenue / ds.kpis.revenue_total * 100).toFixed(1)}%</td></tr>`).join('')}
        </tbody></table>`;
    }

    // === CATEGORÍAS ===
    if (/categor|departa|clase/.test(text)) {
      const cats = ds.categorias.slice(0, 8);
      // Margen por categoría
      const catMargen = cats.map(c => {
        const items = ds.skus.filter(s => s.categoria === c.nombre);
        const mg = items.length ? items.reduce((a,s)=>a+s.margen,0)/items.length : 0;
        return { ...c, margen: mg };
      });
      // Si pregunta por margen específicamente, ordenar por margen
      const orderByMg = /margen|rentab/.test(text);
      const sorted = orderByMg ? [...catMargen].sort((a,b) => b.margen - a.margen) : catMargen;
      return `<strong>Categorías ${orderByMg ? 'por margen' : 'por revenue'}:</strong>
        <table><thead><tr><th>Categoría</th><th>Revenue</th><th>Mg avg</th><th>SKUs</th></tr></thead><tbody>
        ${sorted.slice(0, 8).map(c => `<tr><td>${escapeHtml(c.nombre)}</td><td>${fmt.money(c.revenue)}</td><td>${(c.margen*100).toFixed(1)}%</td><td>${c.skus}</td></tr>`).join('')}
        </tbody></table>`;
    }

    // === ELASTICIDAD ===
    if (/elasticidad|sensibilidad|elastico|elástico|inelastico|inelástico/.test(text)) {
      const propias = ds.skus.filter(s => s.elastSource === 'propia');
      const elastic = ds.skus.filter(s => Math.abs(s.elasticidad) > 1.5).length;
      const inelastic = ds.skus.filter(s => Math.abs(s.elasticidad) < 1).length;
      const avgE = propias.length ? propias.reduce((a,s)=>a+s.elasticidad,0) / propias.length : 0;
      const mostElastic = [...ds.skus].sort((a,b) => Math.abs(b.elasticidad) - Math.abs(a.elasticidad)).slice(0, 5);
      return `<strong>Elasticidad del portafolio:</strong><br>
        Promedio (de SKUs con elast. propia): <em>${avgE.toFixed(2)}</em><br>
        <strong>${inelastic}</strong> inelásticos (|E|&lt;1) · <strong>${elastic}</strong> elásticos (|E|&gt;1.5)<br><br>
        <strong>Top 5 más elásticos:</strong><ul>
        ${mostElastic.map(s => `<li><code>${s.sku}</code> ${escapeHtml(s.nombre).substring(0,30)}: <em>${s.elasticidad.toFixed(2)}</em></li>`).join('')}
        </ul>`;
    }

    // === SKU específico ===
    const skuMatch = q.match(/sku\s*(\w+)|^(\w{4,})$/i);
    if (skuMatch) {
      ...
      if (sku) {
        return `<strong>${escapeHtml(sku.nombre)}</strong> · ${escapeHtml(sku.marca)}<br>
          ...
          💰 Precio: <strong>${fmt.money2(sku.precio)}</strong> · Costo: ${sku.costo ? fmt.money2(sku.costo) : '—'}<br>
          📊 Margen: <strong>${(sku.margen*100).toFixed(1)}%</strong> · Elasticidad: <strong>${sku.elasticidad.toFixed(2)}</strong> (${sku.elastSource})<br>
          📦 Unidades: <strong>${fmt.num(sku.unidades)}</strong> · Demanda: <em>${sku.demanda.replace('_', ' ')}</em><br>
    ...
    // === CONSULTAS NUMÉRICAS ===
    if (/cuantos|cuántos|total/.test(text)) {
      if (/sku/.test(text)) return `Tienes <strong>${fmt.num(ds.meta.skusTotales)} SKUs</strong> activos en tu dataset.`;
      if (/marca/.test(text)) return `Tienes <strong>${ds.kpis.marcas} marcas</strong> distintas.`;
      if (/tienda/.test(text)) return `Tienes <strong>${ds.kpis.tiendas || 0} tiendas/canales</strong>.`;
      if (/transac|ventas/.test(text)) return `<strong>${fmt.num(ds.kpis.transacciones)} transacciones</strong> · <strong>${fmt.num(ds.kpis.unidades)} unidades</strong> vendidas.`;
    }

    // === REVENUE / UTILIDAD ===
    if (/revenue|ingreso|venta total/.test(text)) {
      return `Revenue total: <strong>${fmt.money2(ds.kpis.revenue_total)}</strong> sobre ${fmt.num(ds.kpis.transacciones)} transacciones. Ticket promedio: <strong>${fmt.money2(ds.kpis.ticket_promedio)}</strong>.`;
    }
    if (/utilidad|profit|ganancia/.test(text)) {
      return `Utilidad acumulada: <strong>${fmt.money2(ds.kpis.utilidad_total)}</strong>. Margen promedio: <strong>${(ds.kpis.margen_avg*100).toFixed(1)}%</strong>.`;
    }

    // === HELP / AYUDA ===
    if (/ayuda|help|que puedes|qué puedes|comandos/.test(text)) {
      return `Puedo responderte sobre:<ul>
        <li>📊 <strong>Resumen ejecutivo</strong> ("resumen")</li>
        <li>🏆 <strong>Top SKUs</strong> ("top 5 productos")</li>
        <li>↑ <strong>Oportunidades de precio</strong> ("dónde subir precio")</li>
        <li>⊕ <strong>Candidatos a promo</strong> ("promos 2x1")</li>
        <li>⚠ <strong>Productos en riesgo</strong> ("riesgo crítico")</li>
        <li>📈 <strong>Elasticidad</strong> ("más elásticos")</li>
        <li>🏷 <strong>Marcas/categorías</strong> ("marca más rentable")</li>
        <li>🔍 <strong>SKU específico</strong> (escribe el código)</li>
        </ul>O usa las sugerencias rápidas abajo.`;
    }

    // === DEFAULT ===
    return `Hmm, no entendí tu pregunta. Prueba algo como:<ul>
      <li>"resumen ejecutivo"</li>
      <li>"top 10 SKUs"</li>
      <li>"oportunidades de subir precio"</li>
      <li>"productos en riesgo"</li>
      <li>"marca más rentable"</li>
      </ul>O escribe <em>ayuda</em> para ver todo lo que puedo hacer.`;
  }
