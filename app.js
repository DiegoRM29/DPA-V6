...
const App = (() => {
  ...
  const COL_PATTERNS = {
    ...
    // ORDEN IMPORTA: detectColumns toma la primera columna que matchea.
    // Preferimos categorías más granulares (class_nm, subdept_nm) sobre dept_nm
    // porque dept_nm suele ser un solo valor para datasets filtrados a un departamento.
    // Si todos los SKUs caen en una sola "categoría", todos heredan la misma elasticidad,
    // lo cual hace que el motor de elasticidad sea inútil.
    categoria: [
      /^category$/i, /categoria/i,
      /class[_\s]?nm/i,         // class_nm — más granular (preferida)
      /subdept[_\s]?nm/i,       // subdept_nm — granular media
      /^class$/i,
      /subdept/i,
      /dept[_\s]?nm/i,          // dept_nm — última (suele ser un solo valor)
      /^group/i
    ],
  ...
  function detectColumns(headers) {
    // FIX: Itera PATRONES primero, luego HEADERS, para que el ORDEN de los patrones
    // determine la prioridad. Si el patrón #1 matchea algún header, gana sobre
    // el patrón #2 incluso si #2 matchea un header anterior en el CSV.
    // Importante para 'categoria': preferimos class_nm sobre dept_nm aunque dept_nm
    // venga primero en el CSV.
    const mapping = {};
  ...
  function recommendAction(s) {
    ...
    if (conf === 'Baja') return {
      accion: 'MANTENER', pct: 0,
      razon: `Baja confianza estadística (R²<0.2) en elasticidad. Mantener precio actual; se requiere mayor variación de precio histórica para estimar con seguridad.`
    };
  ...
  function renderPredictive() {
    ...
    // Top más elásticos — filtros mejorados:
    //  1) Solo elasticidades NEGATIVAS (positivas violan la Ley de la Demanda, son ruido/estacionalidad)
    //  2) Solo SKUs con |E| ≥ 1.0 (realmente elásticos, no inelásticos disfrazados)
    //  3) Si menos de 3 SKUs cumplen, mostrar mensaje claro en lugar de relleno engañoso
    //  4) Desempate por revenue cuando muchos SKUs heredan la misma elasticidad
    const realElastics = skus.filter(s => s.elasticidad < 0 && Math.abs(s.elasticidad) >= 1.0);
    const topElast = realElastics
      .sort((a,b) => {
        const dE = Math.abs(b.elasticidad) - Math.abs(a.elasticidad);
        if (Math.abs(dE) > 0.001) return dE;
        return b.revenue - a.revenue; // desempate por revenue (más material primero)
      })
      .slice(0, 8);

    const topElastList = document.getElementById('topElastList');
    if (topElast.length >= 3) {
      topElastList.innerHTML = topElast.map(s => `
        <div class="elast-row">
          <div>
      ...
      // Mensaje honesto: no hay suficiente señal elástica en el portafolio
      const maxEFound = skus.length
        ? Math.max(...skus.filter(s => s.elasticidad < 0).map(s => Math.abs(s.elasticidad)), 0)
        : 0;
      topElastList.innerHTML = `
        <div style="padding: 20px; text-align: center; color: var(--text-3); font-size: 12.5px; line-height: 1.6;">
          <div style="font-size: 24px; opacity: 0.5; margin-bottom: 8px;">⚠</div>
          <strong style="color: var(--text-2);">Sin SKUs realmente elásticos</strong><br>
          Ningún producto cumple |E| ≥ 1 con elasticidad negativa.<br>
          <span style="font-size: 11px;">Mayor |E| observado: ${maxEFound.toFixed(2)}. El portafolio es predominantemente inelástico al precio en el periodo analizado, posiblemente por baja variación de precio en el histórico.</span>
        </div>`;
    }
  ...
  function renderElasticity() {
    ...
    const skusForScatter = skus; // siempre mostrar todas (propias + heredadas)
    ...
    charts.elastScatter = new Chart(document.getElementById('chartElasticScatter'), {
      ...
      options: { responsive: true, maintainAspectRatio: false,
        plugins: {
          ...
          subtitle: {
            display: true,
            text: 'Puntos translúcidos = elasticidad heredada de categoría (menor confianza estadística)',
    ...
    const elastSource = skus; // siempre todas
...
- "MANTENER", "CROSS-SELL", "REVISAR COSTO", "DISCONTINUAR", "EVITAR PROMO": 0.
...
<anomaly>${JSON.stringify(ctxItem, null, 1)}</anomaly>`;
  ...
  function init() {
    ...
    setupPromoUpload();
    setupAIButtons();
    setupApiKeyModal(); // antes lo llamaba setupChat
    updateLLMStatus();  // estado inicial del card de IA
  }
