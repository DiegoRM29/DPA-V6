...
const App = (() => {
  ...
  function setupApiKeyModal() {
    ...
    document.getElementById('apiKeySave').addEventListener('click', async () => {
      ...
      const model = document.getElementById('providerModelSelect').value;
  ...
  function updateLLMStatus() {
    ...
    if (LLM_STATE.key && LLM_STATE.provider) {
      ...
      status.textContent = `${p.icon} ${p.name} · ${LLM_STATE.model}`;
      ...
      status.textContent = 'Modo local · sin IA conectada';
      avatar.textContent = 'AI';
      connectBtn.textContent = '🔌 Conectar IA';
  ...
  function updateCostBar() {
    document.getElementById('chatCostValue').textContent = '$' + SESSION_COST.toFixed(4);
    document.getElementById('chatCostDetail').textContent =
  ...
  // ===== LLAMADA AL LLM (cualquier proveedor) =====
